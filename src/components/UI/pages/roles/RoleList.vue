<template>
  <div>
    <the-header name="header"></the-header>
    <div id="layoutSidenav">
      <side-navbar></side-navbar>
      <div id="layoutSidenav_content">
        <div class="container-fluid px-4">
          <div class="row">
            <div class="col">
              <h3 class="mt-4">Roles</h3>
            </div>
            <div class="col-auto m-4">
              <button class="btn btn-primary" @click="addRole()">
                Add Role
              </button>
            </div>
          </div>
          <div class="card m-4">
            <vue-good-table :columns="columns" :rows="rows" :search-options="{
              enabled: true,
            }" :pagination-options="{
  enabled: true,
}">
              <template v-slot:table-row="props">
                <span v-if="props.column.field == 'action'">
                  {{ props.formattedRow[props.column.field] }}
                  <span v-on:click="editRow(props.row)">
                    <i class="fas fa-edit text-warning mx-1 z-10"></i>
                  </span>
                  <span>|</span>
                  <span @click="deleteRow(props.row)">
                    <i class="fas fa-trash text-danger mx-1"></i>
                  </span>
                </span>
              </template>
            </vue-good-table>
          </div>
        </div>

        <the-footer name="footer"></the-footer>
      </div>
    </div>
  </div>

  <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">{{ modalTitle }}</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <form>
            <div class="mb-2">
              <label for="role">Role :</label>
              <input type="text" class="form-control" id="role" v-model="role" aria-describedby="role"
                placeholder="Enter Role" />
            </div>
          </form>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" @click="closeModal()">
            Close
          </button>
          <button v-if="showAddRole" type="button" class="btn btn-success" @click="saveRole()">
            Add Role
          </button>
          <button v-if="showEditRole" type="button" class="btn btn-success" @click="saveChanges()">
            Update
          </button>
        </div>
      </div>
    </div>
  </div>
</template>
  
<script>
import { VueGoodTable } from "vue-good-table-next";
import axios from "axios";
let mymodal;
let token;

export default {
  data() {
    return {
      modalTitle: "Add Role",
      showEditRole: false,
      showAddRole: true,
      role: '',
      columns: [
        {
          label: "Id",
          field: "id",
        },
        {
          label: "Role Name",
          field: "name",
        },
        {
          label: "Total Employees",
          field: "employees_count",
        },
        {
          label: "Action",
          field: "action",
        },
      ],
      rows: [],
    };
  },
  methods: {
    closeModal() {
      mymodal.hide();
    },
    addRole() {
      this.modalTitle = "Add Role",
        this.showEditRole = false;
      this.showAddRole = true;
      this.role = '';
      mymodal.show();
    },
    saveRole() {
      try {
        const editedRow = {
          role: this.role,
        };

        axios
          .post(`http://127.0.0.1:8000/api/role`, editedRow,
            {
              headers: {
                Accept: 'application/json',
                Authorization: `Bearer ${token}`,
              }
            })
          .then((response) => {
            if (response.data.success) {
              this.rows.push(response.data.data);
              mymodal.hide();
              this.$swal.fire({
                title: "Created!",
                text: response.data.message,
                icon: "success",
              });
            }
          })
          .catch((error) => {
            mymodal.hide();
            this.$swal.fire({
              icon: "error",
              title: "Oops...",
              text: "Something went wrong!",
            });
          });
      } catch (error) {
        this.$swal.fire({
          icon: "error",
          title: "Oops...",
          text: "Something went wrong!",
        });
      }
    },
    editRow(row) {
      this.currentRow = row;
      this.showEditRole = true;
      this.showAddRole = false;
      this.role = row.name;
      mymodal.show();
    },
    saveChanges() {
      try {
        const editedRow = {
          role: this.role,
        };

        axios
          .put(
            `http://127.0.0.1:8000/api/role/${this.currentRow.id}`,
            editedRow,
            {
              headers: {
                Accept: 'application/json',
                Authorization: `Bearer ${token}`,
              }
            }
          )
          .then((response) => {
            if (response.data.success) {
              let rowIndex = this.rows.findIndex(
                (row) => row.id === this.currentRow.id
              );

              if (rowIndex !== -1) {
                this.rows[rowIndex] = response.data.data;
              }
              mymodal.hide();
              this.$swal.fire({
                title: "Updated!",
                text: response.data.message,
                icon: "success",
              });
            }
          })
          .catch((error) => {
            mymodal.hide();
            this.$swal.fire({
              icon: "error",
              title: "Oops...",
              text: "Something went wrong!",
            });
          });
      } catch (error) {
        this.$swal.fire({
          icon: "error",
          title: "Oops...",
          text: "Something went wrong!",
        });
      }
    },
    deleteRow(row) {
      const confirmed = this.$swal({
        title: "Are you sure?",
        text: "You won't be able to revert this!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: "Yes, delete it!",
      }).then((result) => {
        if (result.isConfirmed) {
          axios
            .delete(`http://127.0.0.1:8000/api/role/${row.id}`,
              {
                headers: {
                  Accept: 'application/json',
                  Authorization: `Bearer ${token}`,
                }
              })
            .then((response) => {
              if (response.data.success) {
                const index = this.rows.findIndex((r) => r.id === row.id);
                if (index !== -1) {
                  this.rows.splice(index, 1);
                  this.$swal.fire({
                    title: "Deleted!",
                    text: response.data.message,
                    icon: "success",
                  });
                }
              }
            })
            .catch((error) => {
              mymodal.hide();
              this.$swal.fire({
                icon: "error",
                title: "Oops...",
                text: "Something went wrong!",
              });
            });
        }
      });
    },
  },
  mounted() {
    token = localStorage.getItem('authToken');
    mymodal = new bootstrap.Modal(document.getElementById("exampleModal"));

    axios
      .get("http://127.0.0.1:8000/api/role",
        {
          headers: {
            Accept: 'application/json',
            Authorization: `Bearer ${token}`,
          }
        })
      .then((response) => {
        this.rows = response.data.data;
      })
      .catch((error) => {
        this.error = error.message;
        this.loading = false;
      });
  },
  components: {
    VueGoodTable,
  },
};
</script>
  